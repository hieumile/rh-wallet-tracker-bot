...
1. **Thu thập dữ liệu (Ingestion)**: Mặc định quét trực tiếp On-Chain từ RPC Node của Robinhood Chain để bắt 100% ví giao dịch trong khung giờ, hoặc dùng tùy chọn `--gmgn` để khai thác cơ sở dữ liệu Top Traders của GMGN.
2. **Đánh giá & Lọc ví (Scoring Engine)**: 
   ...
   * Loại bỏ ví MEV/Sandwich (hold dưới 60s), lọc bot giao dịch (số lệnh > 300), áp dụng sàn winrate tối thiểu 50%, tính toán Max Drawdown, Profit Factor, Sharpe Ratio, và tự động truy vết ví mẹ cấp gas (`insider_funder`). Các ví đạt tiêu chuẩn được lưu vào `watchlist.json` và đồng bộ ra `watchlist.xlsx`.
...
graph TD
    ...
    B --> C[Scoring Engine: Lọc Net Buy, Bot, MEV & Winrate 50% + Sharpe + Profit Factor]
    C -->|Gộp & Lưu| D[watchlist.json / watchlist.xlsx]
    C -->|Xuất Báo Cáo| E[Spreadsheets Excel Từng Token]
...
* `--export <path.xlsx>`: Xuất bảng tổng hợp xếp hạng ví ra file Excel (mặc định tự đặt theo token: `wallet_summary_<token>.xlsx`).
* `--txns <path.xlsx>`: Xuất chi tiết tất cả giao dịch ra Excel (mặc định tự đặt theo token: `transactions_<token>.xlsx`).
...
1. **`transactions_<token>.xlsx`** (ví dụ `transactions_pons.xlsx`): Chứa tab giao dịch sạch và tab "Raw Transactions" kiểm toán toàn bộ sự kiện thô (MEV, status). Tự động gộp dữ liệu nếu quét nhiều lần.
2. **`wallet_summary_<token>.xlsx`** (ví dụ `wallet_summary_pons.xlsx`): Bảng xếp hạng chi tiết các ví gom hàng dự án này.
3. **`watchlist.xlsx`**: Tệp xuất danh sách theo dõi Master chứa các ví uy tín nhất (>= 40 điểm), với cột `Seed Tokens` hiển thị tên Token trực quan (`PONS`, `NOCK`, `WALLET`...).
